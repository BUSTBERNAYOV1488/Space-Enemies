...
function em_draw() {
	...
	*/
	...
	//Draw the title
	draw_text_transformed(em_startx,em_starty-em_title_separation,string_hash_to_newline(em_title),em_title_xscale,em_title_yscale,0);
	...
	for (var i=0; i<ds_list_size(em_section_list); i++) {
	    draw_text(em_startx,em_starty+em_pixel_space+string_height(string_hash_to_newline(em_section_list[| 0]))*i,string_hash_to_newline(em_section_list[| i]));
	}
